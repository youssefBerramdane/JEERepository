...
public class TransfertController {
    ...
    @PostMapping(path="/transfertMultipleToWalets",
            consumes= MediaType.APPLICATION_JSON_VALUE,
            produces=MediaType.APPLICATION_JSON_VALUE
    )
    public ResponseEntity<List<Transfert>> transfertMultipleWaletToWalets(@RequestBody TransfertMultipleDto transfertMultipleDto) throws Exception{
        if(transfertMultipleDto.getClientDonneurId().isEmpty() || transfertMultipleDto.getClientBeneficaireIdList().isEmpty()
                || transfertMultipleDto.getMontant()<100)
            throw new RuntimeException("vous oublier des champs obligatoire");

        List<Transfert> transfertEntity = transfertService.transfertMultipleWToWalets(transfertMultipleDto);
